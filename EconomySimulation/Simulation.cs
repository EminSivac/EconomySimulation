...
namespace EconomySimulation
{
    ...
    {
        public Simulation(List<Firma> firmen, List<Mensch> personen, Markt markt, Staat staat, SimConfig config)
        {
            for (int i = 0; i < config.Simulation.Runden; i++)
            {
                ...
                markt.BasisPreis = UpdateBasisPreis(firmen, config.Markt.Marge);
                ...
                // 10. Gewinne ausschütten (50% der Gewinne, wenn Gewinn > 0)

                firmen.ForEach(f =>
                {
                    double gewinn = (f.Kapital * (1 - config.Staat.Koerperschaftssteuersatz)) - (f.KapitalLetzterMonat * (1 - config.Staat.Koerperschaftssteuersatz));

                    if (gewinn > 0)
                    {
                        double ausschüttung = gewinn * 0.5;
                        f.Kapital -= ausschüttung;

                        double proPerson = ausschüttung / personen.Count;
                ...
                //11. Staat gibt Geld zurück (90% des Budgets, gleichmäßig verteilt auf alle Personen)
                double rueckfluss = staat.Budget * 0.9;
                staat.Budget -= rueckfluss;

                double proPerson = rueckfluss / personen.Count;
